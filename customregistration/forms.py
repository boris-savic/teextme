...
class RegistrationForm(forms.Form):
    ...
    phone_number = forms.RegexField(regex=r'^[0-9]+$',
                                max_length=15,
    ...
    password2 = forms.CharField(label="Password confirmation", widget=forms.PasswordInput(render_value=False))
    ...
    def clean_phone_number(self):
        ...
        """
        existing = get_user_model().objects.filter(phone_number__iexact=self.cleaned_data['phone_number'])
        if existing.exists():
            raise forms.ValidationError("A user with that phonenumber already exists.")
        else:
    ...
    def save(self):
        country_code, phone_number, password = self.cleaned_data['country_code'], self.cleaned_data['phone_number'], self.cleaned_data['password1']
        user = get_user_model().objects.create_user(country_code, phone_number, password)
...
class ActivationForm(forms.Form):
    ...
    validation_code = forms.RegexField(regex=r'^[0-9]+$',
                                max_length=5,
                                widget=forms.TextInput(),
    ...
    def clean_validation_code(self):
        ...
        activation = ActivationProfile.objects.filter(user__phone_number=self.cleaned_data['phone_number'], code=self.cleaned_data['validation_code'])
        if activation:
